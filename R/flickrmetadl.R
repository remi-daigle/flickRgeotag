...
flickr.meta.dl <- function(api_key,bbox,extras='geo,tags,date_taken,url_m'){

    pages <- flickr.photos.search(api_key = api_key,bbox=bbox,extras = extras,pagenum=TRUE)
    ...
    photos <- df %>%
        group_by(p) %>%
        do(flickr.photos.search(api_key = api_key,bbox=bbox,extras = extras,page=.$p)) %>%
        ungroup()
